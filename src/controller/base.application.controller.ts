...
export class ApplicationController {
    ...
    async create(req: Request, res: Response, next: NextFunction) {
        try {
            const Data = await this.model.create(req.body.data);
    ...
    async bulkCreate(req: Request, res: Response, next: NextFunction) {
        try {
            const Data = await this.model.bulkCreate(req.body.data);
            if (!Data.length)
                throw new AppError(
    ...
    async update(req: Request, res: Response, next: NextFunction) {
        try {
            const { id } = req.params;
            const [data] = await this.model.update(req.body.data, {
                where: { id },
            });
            if (!data || data === 0)
                throw new AppError(
    ...
    async getData(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const encriptionedData = encrypt(data);
            if (data.length) {
